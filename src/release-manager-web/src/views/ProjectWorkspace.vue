...
import { ArrowLeft, Rocket, Play, Square, RotateCw, Download, Undo2, Save, CheckCircle2, TerminalSquare, Pencil, LoaderCircle, Trash2, RadioTower, FolderSearch } from 'lucide-vue-next'
...
const currentRelease = computed(() => releases.value.find(x => x.id === project.value?.currentReleaseId))
...
async function load() {
  try {
    ...
    if (!comment.value) {
      const active = configs.value.find(x => x.isActive) || configs.value[0]
      json.value = project.value?.appSettingsJson || active?.jsonContent || '{}'
    }
...
async function refreshLiveState() {
  ...
  try {
    if (tab.value === 'release' && selectedTask.value?.id) {
      ...
      ;[project.value, tasks.value, releases.value, runtime.value] = await Promise.all([api(`/projects/${id.value}`), api<any>(`/projects/${id.value}/deployments`).then(x => x.items), api(`/projects/${id.value}/releases`), api(`/projects/${id.value}/runtime`)])
    }
  } catch (e:any) { error.value = e.message }
...
<section v-if="tab==='overview'" class="overview-grid"><article class="hero-status"><span class="eyebrow">CURRENT RELEASE</span><strong>{{currentRelease?.version||'尚未发布'}}</strong><p>SVN {{currentRelease?.svnRevision?'r'+currentRelease.svnRevision:'—'}}</p><div class="hero-facts"><span><small>最近构建</small>{{when(project?.lastBuiltAtUtc)}}</span><span><small>最近打包</small>{{when(project?.lastPackageAtUtc)}}</span></div><div :class="['runtime-orb',runtime.state==='Running'?'on':'']"><i></i>{{statusText(runtime.state)}}</div></article><article class="detail-card"><h3>构建坐标</h3><dl><dt>仓库</dt><dd>{{project?.svnUrl}}</dd><dt>后台项目</dt><dd>{{project?.backendDirectory}}/{{project?.backendProjectFile}}</dd><dt>管理端</dt><dd>{{project?.adminDirectory}} → {{project?.adminOutputDirectory}}</dd><dt>终端</dt><dd>{{project?.terminalDirectory}} → {{project?.terminalOutputDirectory}}</dd></dl><div class="endpoint-block"><span><RadioTower/>Kestrel 监听地址</span><code v-for="endpoint in project?.endpoints||[]" :key="endpoint">{{endpoint}}</code><small v-if="!project?.endpoints?.length">当前版本未配置 Kestrel Endpoints</small></div></article><article class="detail-card activity"><h3>最近活动</h3><div v-for="t in tasks.slice(0,5)" :key="t.id"><i :class="t.status==='Succeeded'?'ok':''"></i><span>{{statusText(t.status)}}</span><small>{{when(t.createdAtUtc)}}</small></div></article></section>
